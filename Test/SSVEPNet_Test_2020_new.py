...
subject_best_epochs = [] # 记录每个subject的最佳精度迭代
...
for fold_num in range(opt.Kf): #第几折
    ...
    subject_best_epochs.append(fold_best_epochs)
...
write_log(f"{'被试编号':<10} | {'平均精度 (Accuracy)':<15}")
write_log("-" * 30)
...
write_log("-" * 30)
write_log(f"{'总平均精度':<10} | {final_mean_accuracy:>14.2%}")
write_log("="*30)
